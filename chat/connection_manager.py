...
class ConnectionManager:
    def __init__(self, redis_url: str = "redis://localhost:6379"):
        ...
        self.TOTAL_CONNECTION_DURATION_KEY = "chat_metrics:total_connection_duration_seconds"
    ...
    async def connect(self, websocket: WebSocket):
        ...
        print(f"Client {client_id} connected. Total active (Redis): {await self.redis_client.scard(self.ACTIVE_CONNECTIONS_SET_KEY)}")
        ...
        if client_id:
            ...
            if connection_data:
                ...
                if connect_time_str:
                    ...
                    await self.redis_client.incrbyfloat(self.TOTAL_CONNECTION_DURATION_KEY, duration_seconds)

                await self.redis_client.delete(f"chat_conn:{client_id}")

            print(f"Client {client_id} disconnected. Total active (Redis): {await self.redis_client.scard(self.ACTIVE_CONNECTIONS_SET_KEY)}")
    ...
    async def send_personal_message(self, message: str, websocket: WebSocket):
        ...
        if client_id:
            try:
                ...
                await self.redis_client.hincrby(f"chat_conn:{client_id}", "messages_sent", 1)
            except Exception as e:
                print(f"Error sending message to {client_id}: {e}")
                await self.disconnect(websocket) 

    async def broadcast(self, message: str):
        active_client_ids = await self.redis_client.smembers(self.ACTIVE_CONNECTIONS_SET_KEY)
    ...
    async def increment_messages_received(self, websocket: WebSocket):
        client_id = self.ws_to_client_id.get(websocket)
        if client_id:
            await self.redis_client.hincrby(f"chat_conn:{client_id}", "messages_received", 1)

    async def get_metrics(self):
        total_open_connections = await self.redis_client.scard(self.ACTIVE_CONNECTIONS_SET_KEY)
        total_closed_connections = int(await self.redis_client.get(self.TOTAL_CLOSED_CONNECTIONS_KEY) or 0)
        total_connection_duration_seconds = float(await self.redis_client.get(self.TOTAL_CONNECTION_DURATION_KEY) or 0.0)

        avg_closed_duration_seconds = (total_connection_duration_seconds / total_closed_connections) \
                                      if total_closed_connections > 0 else 0.0
        ...
        for client_id in active_client_ids:
            ...
            if conn_data:
                ...
                if connect_time_str:
                    connect_time = datetime.fromisoformat(connect_time_str)
                    current_duration_seconds = (datetime.now() - connect_time).total_seconds()
        ...
        return {
            ...
            "total_connection_duration_seconds": round(total_connection_duration_seconds, 2),
            "average_closed_connection_duration_seconds": round(avg_closed_duration_seconds, 2),
